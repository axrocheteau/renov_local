...
def prepare_train_show(df: Dataframe,
    ...
    for i, (col_X_hot, col_X_not_hot, hyperparams, (model_name, model)) in enumerate(zip(col_X_hots, col_X_not_hots, hyperparams_models, models.items())):
        ...
        if smote:
            ...
            best_model, best_score, best_params, scores = train_hyper(
                hyperparams, model, X_original, y_original, cv, random_state, categorical_feature, verbose, scoring)
        ...
        if show:
            if len(models) > 1:
                if len(np.unique(y_original)) > 10:
                    ...
                    show_matrix(y_pred, y_original, ax_result[i], model_name, labels_matrix)
                ...
                if len(np.unique(y_original)) > 10:
                    ...
                    show_matrix(y_pred, y_original, ax_result, model_name, labels_matrix)
                show_hyperparam_opti(scores, hyperparams,
                                     ax_hyper, model_name)
